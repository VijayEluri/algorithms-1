...
public class BoxesDiv2 {
  public int findSize(int[] candyCounts) {
    final int n = candyCounts.length;
    final PriorityQueue<Integer> pq = new PriorityQueue<>();
    for (int i = 0; i < n; i++) {
      pq.offer(nearestPow(candyCounts[i]));
    }

    while (pq.size() > 1) {
      final int newSz = nearestPow(pq.poll() + pq.poll());
      pq.offer(newSz);
    }
    return pq.poll();
  }

  private static int nearestPow(int v) {
    v--;
    v |= v >> 1;
    v |= v >> 2;
    v |= v >> 4;
