...
class FrequencySort {
  static final class Element implements Comparable<Element> {
    ...
    public Element(int _index, int _value, int _frequency) {
      index = _index;
      value = _value;
    ...
    public int compareTo(Element o) {
      if (frequency < o.frequency) {
        ...
        if (index > o.index) {
          ...
          throw new IllegalStateException(this + " and " + o + "shouldn't have the same index");
        }
      }
    }
  }
  ...
  public int[] sort(int[] input) {
    ...
    for (int i = 0; i < input.length; i++) {
      if (freqTable.containsKey(input[i])) {
        ...
        freqTable.put(input[i], new Element(e.index, e.value, e.frequency + 1));
      } else {
        freqTable.put(input[i], new Element(i, input[i], 1));
      }
    }
