...
public class zerosum {
  ...
  public static void main(String[] args)
      throws IOException {
    final Scanner s = new Scanner(new File("src/main/resources/usaco/zerosum.in"));
    final int n = s.nextInt();
    s.close();

    final PrintWriter w = new PrintWriter(new BufferedWriter(new FileWriter(new File("build/zerosum.out"))));
    ...
    if (i == n + 1) {
      ...
      if (i == 1) {
        ...
        if (prevOp != PrevOp.SPACE) {
          ...
          if (i == 2) {
            solve(i + 1, newSum + ii, PrevOp.SPACE, new StringBuilder().append(i - 1).append(' ').append(i));
          } else {
            solve(i + 1, newSum + ii, PrevOp.SPACE,
                new StringBuilder(sb.substring(0, sb.length() - 2)).append('+').append(i - 1).append(' ').append(i));
            solve(i + 1, newSum - ii, PrevOp.SPACE,
                new StringBuilder(sb.substring(0, sb.length() - 2)).append('-').append(i - 1).append(' ').append(i));
          }
        }
