...
public class pprime {
  public static void main(String[] args)
      throws IOException {
    final Scanner s = new Scanner(new File("src/main/resources/usaco/pprime.in"));
    final int a = s.nextInt();
    final int b = s.nextInt();
    s.close();

    final PrintWriter pr = new PrintWriter("build/pprime.out");
    ...
    for (int i = 1; i < 10; i += 2) {
      for (int j = 0; j < 10; j++) {
        for (int k = 0; k < 10; k++) {
          for (int l = 0; l < 10; l++) {
            final int n =
                pow(10, 6) * i + pow(10, 5) * j + pow(10, 4) * k + pow(10, 3) * l + pow(10, 2) * k + 10 * j + i;
            if (n >= a && n <= b && isPrime(n, primes)) {
              pr.println(n);
            }
          }
    ...
    for (int i = 1; i < 10; i += 2) {
      for (int j = 0; j < 10; j++) {
        for (int k = 0; k < 10; k++) {
          for (int l = 0; l < 10; l++) {
            final int n =
                pow(10, 7) * i + pow(10, 6) * j + pow(10, 5) * k + pow(10, 4) * l + pow(10, 3) * l + pow(10, 2) * k
                    + 10 * j + i;
            if (n >= a && n <= b && isPrime(n, primes)) {
              pr.println(n);
            }
