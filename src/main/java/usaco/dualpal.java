...
/*
 * ID: rdsr.me1 PROG: dualpal LANG: JAVA
 */
...
public class dualpal {
  ...
  public static void main(String[] _)
    ...
    try {
      br = new BufferedReader(new FileReader("src/main/resources/usaco/dualpal.in"));
      pw = new PrintWriter("build/dualpal.out");

      final String[] nos = br.readLine().split(" ");
      final int N = Integer.valueOf(nos[0]);
      final int S = Integer.valueOf(nos[1]);

      for (int n = S + 1, count = 0; count < N; n++) {
        int palCount = 0;

        for (int base = 2; base <= 10 && palCount < 2; base++) {
