...
public class Tree {
    ...
    static String asString(Tree root, int paddingLen) {
        ...
        return
        asString(root.right, paddingLen + len + 1) +
        padding + "/\n" +
    ...
    static Tree fromList(List<Integer> l) {
        ...
        return 
        new Tree(l.get(mid), leftChild, rightChild);
    }
    ...
    static boolean isBST(Tree root) {
        ...
        return 
        nodeWithinRange(root.left, Integer.MIN_VALUE, root.data) &&
        nodeWithinRange(root.right, root.data, Integer.MAX_VALUE);
    }

    static private boolean nodeWithinRange(
            Tree node, int minValue, int maxValue) {
        if (node == null)
            return true;
    ...
    static boolean hasSum(Tree root, int s) {
        ...
        for (int i = 0; i <= s - root.data; i++)
            if (hasSum(root.left,  s - i - root.data) &&
                hasSum(root.right, i))
                return true;
    ...
    private void printPathsHelper(
            Tree node, List<Integer> path, int i) {
        if (node == null)
