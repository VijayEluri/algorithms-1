...
public class WindowWasher {
  public int fastest(int width, int height, int[] washTimes) {
    int[][] s = new int[washTimes.length][width + 1];
    Arrays.sort(washTimes);
    
    for (int i = 1; i <= width; i++)
      s[0][i] = washTimes[0] * height * i;

    int min = s[0][width];
    for (int i = 1; i < washTimes.length; i++) {
      for (int j = width; j > i; j--) {
        for (int k = 1; k <= width - i; k++) {
          int timeTaken = Math.max(s[i-1][j-k], washTimes[i] * height * k);
          if (timeTaken > s[i-1][width] && s[i][j] == 0)
            return s[i-1][width];
          if (s[i][j] == 0 || timeTaken < s[i][j])
            s[i][j] = timeTaken;
          else
            break;
        }
        int totaTimeTaken = s[i][width];
        if (totaTimeTaken < min)
          min = totaTimeTaken;
        else
            return min;
      }
